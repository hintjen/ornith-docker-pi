...
## 12. Docker (GPU passthrough, prebuilt artifacts)

A containerized version lives in **`/mnt/archive/ens/docker/`**. It **copies the already-built
artifacts** (no recompile) and serves the model with GPU passthrough. The 20 GB model is
**mounted as a volume**, not baked into the image (keeps the image ~2.85 GB).

### 12a. One-time host prerequisite — NVIDIA Container Toolkit

Docker was installed but had no NVIDIA runtime. Install + register it:
```bash
...
sudo apt-get update && sudo apt-get install -y nvidia-container-toolkit   # installed 1.19.1
sudo nvidia-ctk runtime configure --runtime=docker
sudo systemctl restart docker
docker info | grep -i runtimes        # should now list: nvidia
```

### 12b. Stage artifacts into the build context

```bash
mkdir -p /mnt/archive/ens/docker/artifacts
cp -al /mnt/archive/ens/llama/src/build/bin   /mnt/archive/ens/docker/artifacts/bin   # hardlink, preserves symlinks
cp -al /mnt/archive/ens/tools/node            /mnt/archive/ens/docker/artifacts/node
cp     /mnt/archive/ens/.pi/agent/models.json /mnt/archive/ens/docker/artifacts/models.json
# plus artifacts/serve.sh and artifacts/pi-ornith (in-container path versions; see the dir)
```
...
docker compose up -d --build          # or: docker build -t ornith:latest .
docker compose logs -f                # watch for "server is listening" (~18s load)
...
```bash
# OpenAI API on the host (published port)
...
docker exec ornith pi-ornith -p "fix foo.py"

# The HOST pi-ornith / ~/.pi config also work — both point at localhost:8090.
```

Context/offload are env-controlled: `ORNITH_CTX` (65536 | 131072 | 262144) and `ORNITH_NCMOE`
(0 = full GPU). Set them in `docker-compose.yml` or `-e` on `docker run`. Verified inside the
container: 4090 visible via `nvidia-smi`, 22.0 GB VRAM @ 64K, **~198 tok/s gen**, Pi tool loop works.

### 12e. How GPU/libs resolve (why this base image)

- Base `nvidia/cuda:12.3.2-runtime-ubuntu20.04` → provides `libcudart/libcublas/libcublasLt.12`
  (matches the nvcc 12.3 the binaries were built with) and Ubuntu-20.04 glibc 2.31 (exact match).
- `libgomp1` added via apt (OpenMP); `curl` for the healthcheck.
- `libcuda.so.1` (the **driver**) is **injected at runtime** by nvidia-container-toolkit from the
  host driver — never copied into the image. `ENV NVIDIA_VISIBLE_DEVICES=all` +
  `NVIDIA_DRIVER_CAPABILITIES=compute,utility` make it visible; `--gpus all` activates it.

### 12f. Container file map

| In-container path | Source |
|---|---|
| `/opt/ornith/bin/` | llama.cpp CUDA binaries (`artifacts/bin`) |
| `/opt/ornith/node/` | Node + global `pi` (`artifacts/node`) |
| `/opt/ornith/serve.sh` | server launcher (env-driven) |
| `/opt/ornith/pi-ornith` | Pi launcher |
| `/root/.pi/agent/models.json` | Pi model config |
